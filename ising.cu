...
static void update_red_black_grid(int(*write), const int(*read), int color) {
...
void update(const float temp, int *red_grid, int *black_grid) {
...
// L: grid width
// __global__ void update_red_black_kernel(int *write, const int *read, int
// color,
...
//   d_exp_table); cudaDeviceSynchronize();
//
